/* Formatted on 6/25/2025 4:36:01 PM (QP5 v5.388) */
...
BEGIN
    BEGIN
        ...
        EXCEPTION
            WHEN OTHERS
            THEN
                vErrorMessage := 'Token found problem from table.' || SQLERRM;
                GOTO final_log;
        END;
    ...
    "OrgInfo": {
        ...
        "TotEmployees": "'
            ...
            LOOP
                ...
                THEN
                    vConcation := NULL;
                ELSIF vLoopCount > 1 AND vNm != vLoopCount
                THEN
                    vConcation := ',';
                ELSE
                    vConcation := '';
    ...
    "AccountInfo": {
        ...
        "ConnRoles": [';
            ...
            FOR M
                IN (SELECT v.FIRST_NAME || ' ' || v.LAST_NAME
                               FULL_NAME,
                           TO_CHAR (v.CUST_DOB, 'YYYY-MM-DD')
                               DOB,
                           v.FATHER_NAME,
                           v.MOTHER_NAME,
                           v.MOBILE_NO,
                           ...
                           TO_CHAR (d.ISSUE_DATE, 'YYYY-MM-DD')
                               ISSUE_DATE,
                           TO_CHAR (d.EXPIRE_DATE, 'YYYY-MM-DD')
                               EXPIRE_DATE
            ...
            -------------------------------------------
            ----Product Now Static Cause of Setup------
            -------------------------------------------
    ...
    "OtherInfo": {
      ...
      ';*/
        ...
        EXCEPTION
            WHEN OTHERS
            THEN
                vErrorMessage := 'API calling problem.' || SQLERRM;
                GOTO final_log;
        --RAISE vMyException;
        END;
        ...
        THEN
            vErrorMessage := 'API Response Problem';
            GOTO final_log;
        --RAISE vMyException;
        END IF;
        ...
        BEGIN
            ...
            pCustomerNumber :=
                APEX_JSON.GET_VARCHAR2 (p_path => 'CustomerNumber');
            pAccountNumber :=
                APEX_JSON.GET_VARCHAR2 (p_path => 'AccountNumber');
        EXCEPTION
            WHEN OTHERS
            THEN
                vErrorMessage := 'Response Data Parsing Problem.' || SQLERRM;
                GOTO final_log;
        --RAISE vMyException;
        END;
        ...
        THEN
            vErrorMessage := 'Problem from API-' || vMessage;
            GOTO final_log;
        --RAISE vMyException;
        END IF;
